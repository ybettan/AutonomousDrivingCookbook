...
# read one of the tsv files
sample_tsv_path = os.path.join(RAW_DATA_DIR, 'normal_1/airsim_rec.txt')
sample_tsv = pd.read_csv(sample_tsv_path, sep='\t')
sample_tsv.head()
print("a tsv file for example:\n{}".format(sample_tsv.head()))


# read the image the fit to the first line of the tsv file we red
...
# FIXME: we can make one of the optimization mentioned in 
...
# read the same image with ROI (Range Of Interest)
...
# collect the full non-images dataset
...
# plot the 2 types of driving strategies - {normal, swerve}
min_index = 100
max_index = 1100
...
# plot the number of data points for each driving strategy
...
# draw the data distribution for each driving strategy
